...
Shader "Hidden/EdgeDetect_Coloured" { 
	Properties {
	...
	{	
		...
		half4 withEdgeColor = lerp(_EdgeColor, tex2D(_MainTex, UnityStereoScreenSpaceUVAdjust(i.uv[0].xy, _MainTex_ST)), Sobel);
		half4 onlyEdgeColor = lerp(_EdgeColor, _BgColor, Sobel);
		return lerp(withEdgeColor, onlyEdgeColor, _BgFade);
	...
	half4 fragRobert(v2f i) : SV_Target {				
		...
		half4 onlyEdgeColor = lerp(_EdgeColor, _BgColor, edge);
		return lerp(withEdgeColor, onlyEdgeColor, _BgFade);

		//return lerp(withEdgeColor, _BgColor, _BgFade);
	}
	...
	half4 fragThin (v2f i) : SV_Target
	{
		...
		half4 withEdgeColor = lerp(_EdgeColor, original, edge);
		half4 onlyEdgeColor = lerp(_EdgeColor, _BgColor, edge);
		return lerp(withEdgeColor, onlyEdgeColor, _BgFade);
